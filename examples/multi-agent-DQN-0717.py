...
import numpy as np
import argparse
from sionna_sensing.single_target_sensing.dqn import DQN
from sionna_sensing.dqn_agent import Agent
from sionna_sensing.sensing_env_multi_agent import Environment
...
def store_data(fixed_route=False,is_los=False):
    r'''
    存储数据，数据保存格式 [episode,target_num, (pos_now,crb(BS_num),pow_next)]
    ...
    '''
    try:
        datas = np.load(data_save_path+f'{data_name}.npy')
    except:
    ...
    for episode in range(sample_num): 
        ...
        while True:
            ...
            if all(done): # 所有目标到达终点
                break
        np.save(data_save_path+f'{data_name}.npy',datas) # 保存数据
        t1 = time.time()-t1
        print(f'Time cost per inner-step:{t1/inner_step:.2f}s')
    t = time.time()-t
...
def train(mode:str='offline',data_collection=False):
    r'''
    模型训练
...
def offline_train(RL:DQN,fixed_route=False):
    ...
    '''
    try:
        datas = np.load(data_save_path+f'{data_name}.npy') # 加载数据
    except:
        raise('datas don\'t exist')     
...
def online_train(RL:DQN,show_scene_image=False):
    for episode in range(3010): 
        ...
        if episode >= 10:
            ...
            correct_action_rate = correct_action/inner_step
...
if __name__ == "__main__":
    ...
    data_name = 'datas-0716-single-target'                          # 数据文件名
    model_name = 'dqn-0716-single-target-offline'                   # 模型文件名
    ...
    data_save_path = f'{scene_training_path}/datas/'                # 数据保存路径
    ...
    env = Environment(image_save_path,target_configs,args,**env_configs) # 初始化环境
    
    state_space = env.state_space       # 状态空间
    action_space = env.action_space     # 动作空间
    
    all_agents = [] # 初始化智能体
    for b_idx in range(env.BS_num):
        brain_file = get_name_brain(args, b_idx)
        all_agents.append(Agent(state_space, action_space, b_idx, brain_file, args)) 
