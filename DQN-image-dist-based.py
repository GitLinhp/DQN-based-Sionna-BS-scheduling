...
class DQNnet(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        # input1 = tf.expand_dims(env.normalized_scene_image, axis=0)
        # input1 = tf.repeat(input1, batch_size, axis=0)
        # input1 = self.resNet(input1)
        # input1 = self.flatten(input1)

        # x = self.dense(tf.concat([input1,inputs],axis=1))
        x = self.dense(inputs)
        return self.out(x)
...
class DQN():
    ...
    def choose_action(self, observation):
        ...
        # scene_image = tf.reshape(scene_image, [3, IMAGE_RESOLUTION[1], IMAGE_RESOLUTION[0]])
        # scene_image = tf.transpose(scene_image, [2, 1, 0])
        #scene_image = observation[:IMAGE_RESOLUTION[0]*IMAGE_RESOLUTION[1]*3]
        #scene_image = tf.reshape(scene_image, [IMAGE_RESOLUTION[0], IMAGE_RESOLUTION[1], 3])
        #scene_image = scene_image[np.newaxis, :]
        # print(f'scene_image shape: {scene_image.shape}')
        #pos_now = observation[-3:]
        #pos_now = pos_now[np.newaxis, :]
        # print(f'pos_now shape: {pos_now.shape}')
        if np.random.uniform() < self.epsilon:
            ...
            if self.best_action and np.random.rand() < self.best_prob:
                ...
                #             best_reward = reward
                #             best_action = action
                crbs = env._get_reward(0,method='crb')
                best_action = np.argmax(crbs)
            action = best_action
    ...
    def learn(self):
        ...
        # batch_memory_as_qnext_input1 = batch_memory_as_qnext_input[:, :-3]
        # batch_memory_as_qnext_input2 = batch_memory_as_qnext_input[:, -3:]
        # batch_memory_as_qnext_input = np.reshape(batch_memory[:, -self.num_feature:], [self.batch_size, 3, IMAGE_RESOLUTION[1], IMAGE_RESOLUTION[0]])
        # batch_memory_as_qnext_input = np.transpose(batch_memory_as_qnext_input, [0, 3, 2, 1])
        batch_memory_as_qeval_input = batch_memory[:, :self.num_feature]
        # batch_memory_as_qeval_input1 = batch_memory[:, :-3]
        # batch_memory_as_qeval_input2 = batch_memory[:, -3:]
        # batch_memory_as_qeval_input = np.reshape(batch_memory[:, :self.num_feature], [self.batch_size, 3, IMAGE_RESOLUTION[1], IMAGE_RESOLUTION[0]])
        # batch_memory_as_qeval_input = np.transpose(batch_memory_as_qeval_input, [0, 3, 2, 1])
        ...
        # train
        self.cost = self.eval_net.train_on_batch(batch_memory_as_qeval_input, q_target)
        print(f"rewards: {self.cost}")
        ...
        if self.learn_step_counter % self.replace_target_iter == 0:
            ...
            print(f"model saved, best cost: {self.best_cost}")
    ...
    def save_model(self,path):
        ...
        np.savetxt(f'{path}/rewards.txt',self.cost_his)
...
class Environment():
    def __init__(self,**kwargs):
        ...
        self.frequncy = kwargs.get('frequency',6e9)
    ...
    def get_observation(self):
        ...
        observation = tf.concat([self.pos_now],axis=0)
    ...
    def get_data_label(self,target_info=False):
        ...
        image = self._get_scene_image(filename = "scene_tmp.png")
...
def run():
    ...
    for episode in range(3000):
        ...
        observation = env.reset() # observation为场景图片+目标位置坐标
        inner_step = 0
        while True:
            ...
            print(f"\r【{step}-{inner_step}th step】pos:[{float(env.pos_now[0]):.1f},{float(env.pos_now[1]):.1f},{float(env.pos_now[2]):.2f}]\tBS:{action}({action_type})\treward:{env.reward:.4f}\t{env.los}")
            RL.store_transition(observation, action, reward, observation_)
            if (step >= 80) and (step % RL.batch_size == 0):
                RL.learn()
...
if __name__ == "__main__":
    ...
    env = Environment()
    model_save_path = f'./models/street/dqn-image-pos-2/' # 模型保存路径
    RL = DQN(env.n_features,env.action_space,memory_size=1000,best_action=True,best_prob=0.9,reward_decay=0.9,batch_size=80,replace_target_iter=5)
    run() 
