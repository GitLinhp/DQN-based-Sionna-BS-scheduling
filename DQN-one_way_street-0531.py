...
model_save_path = './DL_workspace/'+scene_name+'/models/' # 模型保存路径
image_save_path = './DL_workspace/'+scene_name+'/images/' # 场景图像保存路径
...
class Environment():
    def __init__(self,**kwargs):
        self.scene = None
        # 环境路径
        self.env_path = kwargs.get('env_path','./scenes/'+scene_name.title()+'/'+scene_name+'.xml')
        # 基站参数---------------------------------------------------------
        # 基站个数
        ...
        self.target_name = kwargs.get('target_name','car_1')
        self.target_path = kwargs.get('target_path','meshes/car_1.ply')
    ...
    def _get_reward(self,action,method='mse',crb_target=None):
        ...
        if method == 'mse':
            ...
            # rewards = np.where(crb_target==1.0,-1,c)
            rewards = c
            return rewards
...
def data_normalization(data, method='min_max'):
    '''数据归一化
        method = 'min_max': 最小最大归一化
        method = 'z_score': 零-均值归一化
    '''
    if method == 'min_max':
        min = np.min(data)
        max = np.max(data)
        data = ((data-min)/(max-min)-0.5)*2
    elif method == 'z_score':
        mean = np.mean(data)
        std = np.std(data)
        data = (data-mean)/std
    elif method == 'log':
        data = np.log10(data)/np.log10(np.max(data))
    return data
...
if __name__ == "__main__":
    # 数据保存路径
    data_path = './DL_workspace/'+scene_name+'/datas/datas-0705.npy'
    ...
    model_save_path = './DL_workspace/'+scene_name+'/models/dqn-0705/' # 模型保存路径
    # 初始化 DQN 网络
