...
class Environment():
    def __init__(self,image_save_path,target_configs,**kwargs):
        self.scene = None
        # 环境路径
        ...
        self.action_space = self.BS_num                 # 动作空间
        # 天线配置参数 ---------------------------------------------------------
        ...
        # 其他参数
        self.DASX = kwargs.get('DASX')
        self.DASY = kwargs.get('DASY')
        ...
        # 特征数量 ---------------------------------------------------------
        self.n_features = self.target_num*2 # 各目标的x,y位置坐标
    ...
    def mk_sionna_env(self):
        ...
        for target in self.targets:
            ...
            target.SceneObject.position = target.initial_position
            target.SceneObject.orientation = target.initial_orientation
        # 配置天线阵列------------------------------------------------
        scene.tx_array = PlanarArray(**self.tx_params)
        scene.rx_array = PlanarArray(**self.rx_params)
        scene.frequency = self.frequency # 以Hz为单位，隐含更新电磁材料特性
    ...
    def _get_scene_image(self,filename:str=None,camera_position=[0,0,1000],camera_look_at=[0,0,0]):
        r'''
    ...
    def step(self, action, crbs):
        ...
        return self.next_observation, reward, self.done
    ...
    # 获取observation，需要修改
    def get_observation(self,show_scene_image=False,observation_mode='pos'):
        r'''
        获取观测值
